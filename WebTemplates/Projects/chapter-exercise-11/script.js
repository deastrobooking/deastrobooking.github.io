...
function getWeather(evt) {
   ...
   // instantiate, open, and submit XHR object
   if (!httpRequest) {
      httpRequest = getRequestObject();
      // cancel any existing HTTP requests before starting new one
      httpRequest.abort();
      // concatenate filename solar.php with query strings and values
      httpRequest.open("get","solar.php?" + "lat=" + latitude + "&lng=" + longitude, true);
      // if specifying GET for open() method, pass value of null to send() method
      httpRequest.send(null);
   }

   // configure fillWeather() function as event handler for readystatechange event
...
function fillWeather() {
  // include conditional code to check for appropriate readyState and status values
  if (httpRequest.readyState === 4 && httpRequest.status === 200) {
    // parse data received from requested XHR object
